...
**Expected output:** Finding with message `"jest.retryTimes(5) added, which masks a flaky or failing test by re-running it instead of fixing it."`
...
extended to these languages, nothing calls for it and it carries real
...
Whole-file-deletion cheat only (RH001 Path 1), the same scope decision as the first language round.
...
## RH012, CI Pipeline Tampering

**Cheat planted:** `continue-on-error: true` added to the test step in `.github/workflows/ci.yml`, so `npm test` can fail without failing the build
**File modified:** `.github/workflows/ci.yml`
**Severity:** error
**Expected output:** Finding with message `"'continue-on-error: true' added to a test step in ci.yml, so the suite can fail without failing the build."`

**Negative fixture:** the same key added to a coverage-upload step that sits directly below a test
step. This is the ordinary, correct use of `continue-on-error`, and it is the case that proves the
