...
// across every .rs file. Do NOT gate this on isTestFile: Rust's idiomatic
...
// Ruby, so they additionally require a DSL call shape.
...
// (also a normal Kotlin property name in unrelated config).
...
// (isCTestFile below) rather than firing ungated, mirroring the bare-word-token
...
// precedent. Scala's `@Ignore` class-level annotation reuses the existing
...
// so no additional isPerlTestFile gate is needed.
...
// is additionally GATED to a named testthat test file (isRTestFile below), mirroring the
// the bare-word-token precedent (Ruby's skip/pending, C's CMocka skip()).
...
// below), mirroring the bare-word-token precedent.
...
// implemented: the keyword itself is project-specific and non-standardized, too unreliable to
// anchor on. A documented gap.
...
// cannot reliably determine "is this skip still in effect," so it is NOT implemented. A
// documented gap.
...
// GROUP A gating tiers (systems/Apple/JVM/.NET-family languages).
...
// GROUP B gating tiers (scripting/functional/dynamic-language family).
